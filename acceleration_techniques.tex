...
As only partitioned approaches are being considered, the thermal and mechanical problems are solved separately, i.e., Equation~\eqref{eq:mech_problem} is solved considering a fixed temperature, and Equation~\eqref{eq:therm_problem} is solved assuming a fixed configuration.
...
The reason for this choice of subscripts is made clear in Section~\ref{sec:fixed_point_approachd}.
...
\jvc{Explain a difference vector/scalar}

\section{A classification scheme for iterative methods}
\jvc{Should we include more classification schemes? Classify by order also? And so on?}
...
Calling it an Aitken method is perhaps a misnomer since, in the Aitken-Steffson method, the function values are computed twice per iteration.
