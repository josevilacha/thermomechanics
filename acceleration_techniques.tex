...
More precisely, the quantity to be approximated is
\begin{equation}
...
\end{equation}
assuming \(\mathbf r^{k+1}\) and thus \(\Delta \mathbf r^k = \mathbf r^{k+1} - \mathbf r^k = 0 -\mathbf r^k\).

Since computing \(\mathcal R\) is expensive the estimate for the inverse of the Jacobian tries to take the most advantage from the values already computed so far during the iterative procedure.
...
To avoid solving the linear system, the vector \(\hat{J}_\pazocal{R}(\uptheta^k)\Delta \mathbf r^k\) is considered instead, yielding
\begin{equation}
...
A reasonable choice for \(\mathbf c\) will be one that leads to a good approximation of \(\Delta \mathbf r^k\) by the available \(\Delta \mathbf r^i\), e.g., the minimization of 
