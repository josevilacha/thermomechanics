...
Following \cite{dennis}, the terms "global," as in "global method" or "globally convergent algorithm" are here used to denote a method that is designed to converge to a local minimizer of a nonlinear functional or some solution of a system of nonlinear equations, from almost any starting point.
...
It exposition follows \cite{dennis} where more details can be found.
...
If the Jacobian of \(\mathcal R\) is not available and its estimate is of poor quality the global procedure may be compromised \cite{kelley}.
...
See \cite{dennis} for a full discussion on the choice of \(\lambda_k\).
...
For the details on the choice of \(\delta^k\) see \cite{dennis}.
...
See Box~\ref{arnoldi_process} for the pseudo-code of the Newton-Arnoldi and Newton-GMRES.
...
Since the present use-case includes a large amount of unknows it leads to memory concerns if the Krylov subspace is allowed to grow indefinitely.
...
As a simple approach, \cite{kelley} suggest \(\eta=0.1\), however he describes more sophisticated ways to choose this parameter.
...
\begin{table}[htbp]
  \caption{Summary of the comparison between the FFT-Galerkin method. \(n\) here denotes the number of unknowns and \(m\) denotes depending on the context the number of previous iterates considered, the number of fixed point evaluations or the size of the Krylov subspace.}
\label{tab:comparison_fft_galerkin_fem}
  ...
  \centering
    ...
    & & & \textbullet\\
    \hline
    \multirow{3}{*}{Newton-Krylov} & \multirow{3}{*}{\makecell[c]{Up to\\\((m+1)\) \((n\times 1)\) vectors}} & \multirow{3}{*}{\(-^*\)} & \textbullet \\
    & & & \textbullet\\
    & & & \textbullet\\
    \hline
