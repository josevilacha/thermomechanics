...
\end{equation}
To make some explanation furhter down the line simpler, consider also the fucntion
\begin{gather}
...
Newton-Raphson or Newton scheme is a very popular iterative solution procedure for nonlinear systems of equations, which under appropriate conditions converges quadratically.
It can be applied to Equation~\eqref{eq:residual_fixed_point} yielding
\begin{highlight}[innertopmargin=-5pt]
  \begin{gather}
...
To ease the explanation, consider, a thermal residual operator $\pazocal{R}_{u}(\mathbf{u}, \bm{\uptheta})$ and a mechanical residual operator $\pazocal{R}_{\theta}(\mathbf{u}, \bm{\uptheta})$ defined to be the first and second components in the definition of \(\pazocal R_\mathrm{GS}\) (Equation~\eqref{eq:def_res_gauss_seidel}).
Written in full
\begin{gather}
...
\end{align}
Thus, the Jacobians now needed are now \(J_\pazocal{U}\) and \(J_\pazocal{T}\).
...
Now, a general vector $\mathbf{D}$ is introduced, which can either stand for a solution vector, respectively.
The next step is to work out the solution to the current iteration from the outcome of the previous iteration $\mathbf{D}^{(k)}$ plus a new increment $\Delta \mathbf{D}^{(k)}$
\begin{equation}
\mathbf{D}^{(k+1)}=\mathbf{D}^{(k)}+\Delta \mathbf{D}^{(k)}.
\end{equation}
The increment reads
\begin{equation}
\Delta \mathbf{D}^{(k)}=\omega^{(k)}\left(\pazocal S(\mathbf{D}^{(k)})-\mathbf{D}^{(k)}\right)=\omega^{(k)} \mathbf r^{(k)}.
\end{equation}
