...
\subsection{First principle of thermodynamics}
...
Moreover the terms in the inequalitiy can be splited into the mechanical internal dissipation \(\pazocal D_\text{mech}\) and the dissipation due to heat conduction \(\pazocal D_\text{cond}\).
From
\begin{equation}
...
If on the other hand, only conduction effects are retained, i.e., assuming the process to be isothermic, isochoric and isovolumetric, the dissipation due to conduction, \(\pazocal D_\text{cond}\), is obtained as \jvc{Not 100\% on this}
\begin{equation}
